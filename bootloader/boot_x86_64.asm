; boot_x86_64.asm - 512-byte bootloader for Nano-Alpine Linux (x86_64)
; Loads bzImage (x86_64) + initrd.cpio.xz from raw disk sectors.
;
; Layout on disk (raw image):
;   Sector 0:         This bootloader (boot.bin)
;   Sector 1+:        bzImage (x86_64 kernel)
;   After bzImage:    initrd.cpio.xz
;
; The kernel setup header transitions to 64-bit long mode automatically.
; Patch kernel_start_lba, initrd_start_lba, initrd_size_sectors before writing to disk.
;
; Assemble with: nasm -f bin -o boot_x86_64.bin boot_x86_64.asm
...
.got_setup:
    ...
    ; --- 4. Compute payload (protected/long mode) size in sectors ---
    ; For x86_64: syssize field contains size of 64-bit kernel payload
    ...
    ; --- 5. Load 64-bit kernel payload to 1 MiB (0x100000) ---
    ...
    ; --- 6. Load initrd to 2 MiB (0x200000) via EDD 2.0 DAP (64-bit addr) ---
    mov si, dap_initrd
    ...
    mov bx, 0x9000
    mov es, bx
    mov dword [es:0x218], 0x00200000    ; ramdisk_image = 2 MiB (linear)
    ...
    ; Kernel setup takes over, enters protected mode, then long mode (x86_64)
    mov dl, [boot_drive]
    jmp 0x9020:0
...
dap_initrd:               ; EDD 2.0 extended packet (24 bytes)
    db 0x18                ; packet size = 24
    db 0                   ; reserved
    dw 0                   ; sector count (patched)
    dd 0, 0                ; seg:off ignored in 64-bit flat mode
    dq 0x200000            ; 64-bit flat buffer address = 2 MiB
    dq 0                   ; LBA (patched: low 32 bits at +16, high always 0)
...
initrd_size_sectors dw 0      ; initrd size in 512-byte sectors (e.g. 1228 for 614KB)
